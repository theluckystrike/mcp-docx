...
// Contract suite for docx. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/docx/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-docx-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "docx");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "business.json"), GARBAGE);

  await c.init();
  const r = await c.call("business_set", {"name":"Zovo Studio"});
  ...
  const moved = readdirSync(dir).filter((f) => f.startsWith("business.json.corrupt-"));
  ...
  assert.equal(readdirSync(dir).includes("business.json"), false, "an empty store must not be written over the corrupt one");
});

test.skip("cap: no row-capped write tool on this server", { skip: "docx has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
